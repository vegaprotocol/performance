...
echo TIMESTAMP,VEGAVERSION,VEGABRANCH,TESTNAME,LPUSERS,SLALEVELS,SLAUPDATE,NORMALUSERS,MARKETS,VOTERS,PEGGED,STOPORDERS,USELP,PRICELEVELS,FILLPL,RUNTIME,OPS,EPS,BACKLOG,CORECPU,DNCPU,PGCPU > $PERFHOME/results/all.csv

## Loop through the list of scenarios we want to test
while read -r TESTNAME LPUSERS SLALEVELS SLAUPDATE NORMALUSERS MARKETS VOTERS PEGGED STOPORDERS USELP PRICELEVELS FILLPL RUNTIME OPS || [ -n "$TESTNAME" ]
do
  ...
  SQL="insert into perf_results (TS,VEGAVERSION,VEGABRANCH,TESTNAME,LPUSERS,SLALEVELS,SLAUPDATE,NORMALUSERS,MARKETS,VOTERS, \
    PEGGED,STOPORDERS,USELP,PRICELEVELS,FILLPL,RUNTIME,OPS,EPS,BACKLOG,CORECPU,DNCPU,PGCPU) \
