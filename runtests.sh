...
echo TIMESTAMP,VEGAVERSION,VEGABRANCH,TESTNAME,LPUSERS,SLALEVELS,SLAUPDATE,NORMALUSERS,MARKETS,VOTERS,PEGGED,STOPORDERS,USELP,PRICELEVELS,FILLPL,RUNTIME,OPS,EPS,BACKLOG,CORECPU,DNCPU,PGCPU,USEBATCH,BATCHSIZE > $PERFHOME/results/all.csv

## Loop through the list of scenarios we want to test
while read -r TESTNAME LPUSERS SLALEVELS SLAUPDATE NORMALUSERS MARKETS VOTERS PEGGED STOPORDERS USELP PRICELEVELS FILLPL RUNTIME OPS USEBATCH BATCHSIZE || [ -n "$TESTNAME" ]
do
  ...
  vegatools perftest -a=localhost:3027 -w=localhost:1789 -f=localhost:1790 -c$OPS -r$RUNTIME -u$LPUSERS -l$SLALEVELS -S$SLAUPDATE -n$NORMALUSERS -g=localhost:8545 -m$MARKETS -v$VOTERS -U=$USELP -L$PRICELEVELS -d=0 -B=$USEBATCH -b$BATCHSIZE -t=$PERFHOME/tokens.txt -I >> $PERFHOME/logs/perftest.log 2>&1 &
  ...
  echo $TIMESTAMP,$VEGAVERSION,$VEGABRANCH,$TESTNAME,$LPUSERS,$SLALEVELS,$SLAUPDATE,$NORMALUSERS,$MARKETS,$VOTERS,$PEGGED,$STOPORDERS,$USELP,$PRICELEVELS,$FILLPL,$RUNTIME,$OPS,$USEBATCH,$BATCHSIZE,$EPS,$BACKLOG,$CORECPU,$DNCPU,$PGCPU >> $PERFHOME/results/all.csv
  echo
  ...
  SQL="insert into perf_results (TS,VEGAVERSION,VEGABRANCH,TESTNAME,LPUSERS,SLALEVELS,SLAUPDATE,NORMALUSERS,MARKETS,VOTERS, \
    PEGGED,STOPORDERS,USELP,PRICELEVELS,FILLPL,RUNTIME,OPS,USEBATCH,BATCHSIZE,EPS,BACKLOG,CORECPU,DNCPU,PGCPU) \
