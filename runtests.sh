...
echo TIMESTAMP,VEGAVERSION,VEGABRANCH,TESTNAME,LPUSERS,SLALEVELS,SLAUPDATE,NORMALUSERS,MARKETS,VOTERS,PEGGED,USELP,PRICELEVELS,FILLPL,RUNTIME,OPS,EPS,BACKLOG,CORECPU,DNCPU,PGCPU > $PERFHOME/results/all.csv

## Loop through the list of scenarios we want to test
while read -r TESTNAME LPUSERS SLALEVELS SLAUPDATE NORMALUSERS MARKETS VOTERS PEGGED USELP PRICELEVELS FILLPL RUNTIME OPS || [ -n "$TESTNAME" ]
do
  ...
  vegatools perftest -a=localhost:3027 -w=localhost:1789 -f=localhost:1790 -u$LPUSERS -l$SLALEVELS -n$NORMALUSERS -g=localhost:8545 -m$MARKETS -v$VOTERS -p$PEGGED -U=$USELP -F=$FILLPL -L$PRICELEVELS -t=$PERFHOME/tokens.txt -i >> $PERFHOME/logs/perftest.log 2>&1

  ## Kick off the actual test
  vegatools perftest -a=localhost:3027 -w=localhost:1789 -f=localhost:1790 -c$OPS -r$RUNTIME -u$LPUSERS -l$SLALEVELS -S$SLAUPDATE -n$NORMALUSERS -g=localhost:8545 -m$MARKETS -v$VOTERS -U=$USELP -L$PRICELEVELS -t=$PERFHOME/tokens.txt >> $PERFHOME/logs/perftest.log 2>&1 &
